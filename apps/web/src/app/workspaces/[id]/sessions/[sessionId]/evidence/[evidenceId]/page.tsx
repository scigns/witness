...
export default function EvidenceDetailPage({
  ...
  // Revoke the object URL whenever it changes or the page unmounts — the
  ...
  const uploadAttachment = async () => {
    ...
    try {
      const uploaded = await api.uploadEvidenceAttachment(
        workspaceId,
        sessionId,
        evidenceId,
        attachmentFile,
        ...
        <Card className="space-y-3">
          {evidence.attachment !== null ? (
            <div className="space-y-3 text-sm">
              <p>
                <span className="font-medium">{evidence.attachment.originalFilename}</span>{' '}
              ...
              <details className="mt-3">
                <summary className="cursor-pointer text-sm text-[var(--color-ink-muted)] underline hover:no-underline">
                  File integrity details
                </summary>
                <div className="mt-2 rounded border border-[var(--color-line)] bg-[var(--color-paper)] p-3">
                  <p className="text-xs text-[var(--color-ink-muted)]">
                    Witness keeps an integrity fingerprint for the original file so later changes
                    can be detected without altering the source evidence.
                  </p>
                  <p className="mt-2 break-all font-mono text-xs text-[var(--color-ink-muted)]">
                    SHA-256 {evidence.attachment.checksumSha256}
                  </p>
                </div>
              </details>
            </div>
        ...
        <section aria-labelledby="transcript-heading">
          ...
          <Card className="space-y-3">
            {transcript === null ? (
              <div className="space-y-2">
                <p className="text-sm text-[var(--color-ink-muted)]">
                  No transcript yet. Transcription runs inside the Witness environment, so the
                  recording is not sent to an external AI service.
                </p>
                <Button
                  variant="primary"
              ...
              <p className="text-sm text-[var(--color-ink-muted)]" role="status">
                ...
                minutes depending on the recording length.
              </p>
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="inline-flex items-center rounded-full border border-current px-2 py-0.5 text-xs font-medium">
                      {transcript.confirmed ? 'Human confirmed' : 'Human review required'}
                    </span>
                  </div>
                  ...
                  <details>
                    ...
                    <p className="mt-2 text-xs text-[var(--color-ink-muted)]">
                      Transcription model: <span className="font-mono">{transcript.model}</span>
                    </p>
                  </details>
                </div>
                {editingTranscript ? (
