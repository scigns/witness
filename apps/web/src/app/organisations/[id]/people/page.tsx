...
/**
 * Organisation dashboard — People.
 *
 * Membership/role management, split out of the old all-in-one organisation
 * page. `permittedActions` comes from the server, same reasoning as the
 * record detail page: the client renders what the domain says is possible
 * rather than reimplementing the membership state machine.
 */
...
export default function OrganisationPeoplePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const load = useCallback(
    async (cancelledRef: { current: boolean }) => {
      try {
        ...
        try {
          // Reads the platform-wide user directory, which today's domain
          // model has no role for anyone to hold (see
          // `RoleResolutionService.globalGrantTiers` and `prisma/invite.ts`)
          // — an organisation admin gets `FORBIDDEN` here even though
          // everything else on this page is properly in their scope.
        ...
        <Card className="space-y-3">
          {usersUnavailable ? (
            <p className="text-sm text-[var(--color-ink-muted)]">
              ...
              Witness account into this organisation; use the{' '}
              <Link href={`/organisations/${id}/invitations`} className="underline">
                Invitations
              </Link>{' '}
              tab for anyone new.
            </p>
            ...
            <p className="text-sm text-[var(--color-ink-muted)]">
              Every registered user is already a member, or none exist yet. Use the{' '}
              <Link href={`/organisations/${id}/invitations`} className="underline">
                Invitations
              </Link>{' '}
              tab to register someone new.
            </p>
          ) : (
