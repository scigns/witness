...
export async function fetchPendingOutbox(
  ...
  const { rows } = await pool.query<{
    ...
    `SELECT o.id AS outbox_id, o.event_id, e.event_type, e.aggregate_id, o.attempts
     FROM outbox o
...
export async function loadAssertionProjection(
  ...
  if (attributeRow !== undefined) {
    return {
      ...
      entity: toEntityRow(attributeRow, 'e_'),
  ...
  if (relRow !== undefined) {
    return {
      ...
      fromEntity: toEntityRow(relRow, 'from_'),
      toEntity: toEntityRow(relRow, 'to_'),
    };
  }
